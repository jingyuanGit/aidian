...
export class McpTestModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('aidian-mcp-test-modal');
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-loading' });

    const spinnerEl = loadingEl.createDiv({ cls: 'aidian-mcp-test-spinner' });
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'aidian-mcp-test-icon' });
    ...
    const textEl = statusEl.createSpan({ cls: 'aidian-mcp-test-text' });
    if (this.result.success) {
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-error' });
    ...
    if (this.result.tools.length > 0) {
      const toolsSection = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-tools' });

      const toolsHeader = toolsSection.createDiv({ cls: 'aidian-mcp-test-tools-header' });
      toolsHeader.setText(`Available Tools (${this.result.tools.length})`);

      const toolsList = toolsSection.createDiv({ cls: 'aidian-mcp-test-tools-list' });

      for (const tool of this.result.tools) {
        this.renderTool(toolsList, tool);
      }
    } else if (this.result.success) {
      const noToolsEl = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-no-tools' });
      noToolsEl.setText('No tools information available. Tools will be loaded when used in chat.');
    }

    const buttonContainer = this.contentEl_.createDiv({ cls: 'aidian-mcp-test-buttons' });

    if (this.result.tools.length > 0 && this.onToolToggle) {
      this.toggleAllBtn = buttonContainer.createEl('button', {
        cls: 'aidian-mcp-toggle-all-btn',
      });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'aidian-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'aidian-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'aidian-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'aidian-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'aidian-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'aidian-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('aidian-mcp-test-tool-disabled', !enabled);
  }
