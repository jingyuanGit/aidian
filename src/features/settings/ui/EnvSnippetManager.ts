...
import type { AidianView } from '../../chat/AidianView';
...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('aidian-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      if (uniqueModelIds.size === 0) {
        contextLimitsContainer.addClass('aidian-hidden');
        return;
      }

      contextLimitsContainer.removeClass('aidian-hidden');
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'aidian-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'aidian-snippet-limit-model' });
        row.createSpan({ cls: 'aidian-snippet-limit-spacer' });

        const aliasInput = row.createEl('input', {
          type: 'text',
          placeholder: t('settings.customModelAliases.placeholder'),
          cls: 'aidian-snippet-alias-input',
        });
        ...
        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'aidian-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('aidian-env-snippet-setting');
    envVarsSetting.controlEl.addClass('aidian-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'aidian-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'aidian-snippet-buttons' });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'aidian-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'aidian-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'aidian-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'aidian-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'aidian-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'aidian-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'aidian-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'aidian-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'aidian-snippet-actions' });
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('aidian-view')[0]?.view as AidianView | undefined;
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.aidian-settings-env-textarea[data-env-scope="${scope}"]`;
