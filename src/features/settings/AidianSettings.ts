...
function addHotkeySettingRow(
  ...
  const item = containerEl.createDiv({ cls: 'aidian-hotkey-item' });
  item.createSpan({
    cls: 'aidian-hotkey-name',
    text: t(`${translationPrefix}.name` as TranslationKey),
  });
  if (hotkey) {
    item.createSpan({ cls: 'aidian-hotkey-badge', text: hotkey });
  }
  item.addEventListener('click', () => openHotkeySettings(app));
}

export class AidianSettingTab extends PluginSettingTab {
  plugin: AidianPlugin;
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.addClass('aidian-settings');
    ...
    const tabBar = containerEl.createDiv({ cls: 'aidian-settings-tabs' });
    ...
    for (const id of tabIds) {
      ...
      const button = tabBar.createEl('button', {
        cls: `aidian-settings-tab${id === this.activeTab ? ' aidian-settings-tab--active' : ''}`,
        text: label,
      });
      button.addEventListener('click', () => {
        this.activeTab = id;
        for (const tabId of tabIds) {
          tabButtons.get(tabId)?.toggleClass('aidian-settings-tab--active', tabId === id);
          tabContents.get(tabId)?.toggleClass('aidian-settings-tab-content--active', tabId === id);
        }
      });
    ...
    for (const id of tabIds) {
      const content = containerEl.createDiv({
        cls: `aidian-settings-tab-content${id === this.activeTab ? ' aidian-settings-tab-content--active' : ''}`,
      });
  ...
  private renderGeneralTab(container: HTMLElement): void {
    ...
    const maxTabsWarningEl = container.createDiv({
      cls: 'aidian-max-tabs-warning aidian-setting-validation aidian-setting-validation-warning aidian-hidden',
    });
    maxTabsWarningEl.setText(t('settings.maxTabs.warning'));

    const updateMaxTabsWarning = (value: number): void => {
      maxTabsWarningEl.toggleClass('aidian-hidden', value <= 5);
    };
    ...
    new Setting(container)
      ...
      .addText((text) => {
        ...
        text.inputEl.addClass('aidian-settings-media-input');
        text.inputEl.addEventListener('blur', () => {
          void this.restartServiceForPromptChange();
        });
      });
    ...
    const hotkeyGrid = container.createDiv({ cls: 'aidian-hotkey-grid' });
    addHotkeySettingRow(hotkeyGrid, this.app, 'aidian:inline-edit', 'settings.inlineEditHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'aidian:open-view', 'settings.openChatHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'aidian:new-session', 'settings.newSessionHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'aidian:new-tab', 'settings.newTabHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'aidian:close-current-tab', 'settings.closeTabHotkey');
  ...
  private renderCustomContextLimits(container: HTMLElement, providerId?: ProviderId): void {
    ...
    const headerEl = container.createDiv({ cls: 'aidian-context-limits-header' });
    headerEl.createSpan({
      text: t('settings.customModelOverrides.name'),
      cls: 'aidian-context-limits-label',
    });

    const descEl = container.createDiv({ cls: 'aidian-context-limits-desc' });
    descEl.setText(t('settings.customModelOverrides.desc'));

    const listEl = container.createDiv({ cls: 'aidian-context-limits-list' });
    ...
    for (const modelId of uniqueModelIds) {
      ...
      const itemEl = listEl.createDiv({ cls: 'aidian-context-limits-item' });
      const nameEl = itemEl.createDiv({ cls: 'aidian-context-limits-model' });
      nameEl.setText(modelId);

      const inputWrapper = itemEl.createDiv({ cls: 'aidian-context-limits-input-wrapper' });
      const aliasInputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: t('settings.customModelAliases.placeholder'),
        cls: 'aidian-context-alias-input',
        value: currentAlias,
      });
      ...
      const inputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: '200k',
        cls: 'aidian-context-limits-input',
        value: currentValue ? formatContextLimit(currentValue) : '',
      });
      inputEl.setAttribute('aria-label', `Context window for ${modelId}`);

      const validationEl = inputWrapper.createDiv({ cls: 'aidian-context-limit-validation aidian-hidden' });
      ...
      const saveContextLimit = async (): Promise<void> => {
        ...
        if (!trimmed) {
          ...
          inputEl.classList.remove('aidian-input-error');
        } else {
          const parsed = parseContextLimit(trimmed);
          if (parsed === null) {
            validationEl.setText(t('settings.customContextLimits.invalid'));
            validationEl.toggleClass('aidian-hidden', false);
            inputEl.classList.add('aidian-input-error');
            return;
          }
