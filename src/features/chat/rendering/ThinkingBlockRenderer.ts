...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'aidian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'aidian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'aidian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'aidian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.aidian-thinking-header');
  if (header) {
