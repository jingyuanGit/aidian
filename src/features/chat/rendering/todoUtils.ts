...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `aidian-todo-item aidian-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'aidian-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'aidian-todo-text' });
