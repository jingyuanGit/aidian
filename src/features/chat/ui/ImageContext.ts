...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.aidian-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'aidian-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.aidian-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'aidian-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'aidian-drop-content' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.aidian-input-wrapper');
    if (!inputWrapper) {
  ...
  private updateImagePreview() {
    ...
    if (this.attachedImages.size === 0) {
      this.imagePreviewEl.removeClass('aidian-visible-flex');
      this.imagePreviewEl.addClass('aidian-hidden');
      return;
    }

    this.imagePreviewEl.addClass('aidian-visible-flex');
    this.imagePreviewEl.removeClass('aidian-hidden');
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'aidian-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'aidian-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'aidian-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'aidian-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'aidian-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'aidian-image-remove' });
    removeEl.setText('\u00D7');
