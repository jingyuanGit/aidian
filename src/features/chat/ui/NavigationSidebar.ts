...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'aidian-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('aidian-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('aidian-nav-btn-prev', 'chevron-up', 'Previous message');
    this.nextBtn = this.createButton('aidian-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('aidian-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `aidian-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.aidian-message-user'));
