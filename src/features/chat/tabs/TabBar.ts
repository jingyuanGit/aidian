...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('aidian-tab-badges');
  }
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'aidian-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'aidian-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'aidian-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'aidian-tab-badge-streaming';
    }

    const badgeEl = this.containerEl.createDiv({
      cls: `aidian-tab-badge ${stateClass}`,
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('aidian-tab-badges');
  }
}
