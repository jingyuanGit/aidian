...
import { VIEW_TYPE_AIDIAN } from '../../core/types';
...
export class AidianView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: AidianPlugin) {
    ...
    // overwritten by prototype patching. Hover Editor patches AidianView.prototype.load
  ...
  async onOpen() {
    ...
    this.viewContainerEl.addClass('aidian-container');

    const header = this.viewContainerEl.createDiv({ cls: 'aidian-header' });
    this.buildHeader(header);

    this.navRowContent = this.buildNavRowContent();
    this.tabContentEl = this.viewContainerEl.createDiv({ cls: 'aidian-tab-content-container' });
  ...
  private buildHeader(header: HTMLElement) {
    ...
    this.titleSlotEl = header.createDiv({ cls: 'aidian-title-slot' });

    // Logo (hidden when 2+ tabs) — populated by syncHeaderLogo()
    this.logoEl = this.titleSlotEl.createSpan({ cls: 'aidian-logo' });
    this.syncHeaderLogo(DEFAULT_CHAT_PROVIDER_ID);

    // Title text (hidden in header mode when 2+ tabs)
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: 'Aidian', cls: 'aidian-title-text' });

    // Header actions container (for header mode - initially hidden)
    this.headerActionsEl = header.createDiv({ cls: 'aidian-header-actions aidian-header-actions-slot aidian-hidden' });
  }
  ...
  private buildNavRowContent(): HTMLElement {
    ...
    this.tabBarContainerEl.className = 'aidian-tab-bar-container';
    ...
    this.headerActionsContent.className = 'aidian-header-actions';

    // New tab button (plus icon)
    this.newTabButtonEl = this.headerActionsContent.createDiv({ cls: 'aidian-header-btn aidian-new-tab-btn' });
    ...
    const newBtn = this.headerActionsContent.createDiv({ cls: 'aidian-header-btn' });
    ...
    // History dropdown
    const historyContainer = this.headerActionsContent.createDiv({ cls: 'aidian-history-container' });
    const historyBtn = historyContainer.createDiv({ cls: 'aidian-header-btn' });
    setIcon(historyBtn, 'history');
    historyBtn.setAttribute('aria-label', 'Chat history');

    this.historyDropdown = historyContainer.createDiv({ cls: 'aidian-history-menu' });
    ...
    wrapper.className = 'aidian-input-nav-content';
  ...
  private updateNavRowLocation(): void {
    ...
    if (isHeaderMode) {
      ...
      if (this.headerActionsEl) {
        ...
        this.headerActionsEl.removeClass('aidian-hidden');
      }
    } else {
      ...
      if (this.headerActionsEl) {
        this.headerActionsEl.addClass('aidian-hidden');
      }
    }
  }
  ...
  updateLayoutForPosition(): void {
    ...
    this.viewContainerEl.toggleClass('aidian-container--header-mode', isHeaderMode);
  ...
  private updateTabBarVisibility(): void {
    ...
    this.tabBarContainerEl.toggleClass('aidian-hidden', !showTabBar);
    ...
    if (this.logoEl) {
      this.logoEl.toggleClass('aidian-hidden', hideBranding);
    }
    if (this.titleTextEl) {
      this.titleTextEl.toggleClass('aidian-hidden', hideBranding);
    }
  ...
  private updateNewTabButtonVisibility(): void {
    ...
    this.newTabButtonEl.toggleClass('aidian-hidden', !canCreateTab);
    if (canCreateTab) {
