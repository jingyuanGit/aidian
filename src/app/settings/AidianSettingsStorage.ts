import {
  AIDIAN_SETTINGS_PATH,
  LEGACY_AIDIAN_SETTINGS_PATH,
...
import { DEFAULT_AIDIAN_SETTINGS } from './defaultSettings';
...
export type StoredAidianSettings = AidianSettings;
...
function normalizeChatViewPlacement(
  ...
  return DEFAULT_AIDIAN_SETTINGS.chatViewPlacement;
}
...
export class AidianSettingsStorage {
  constructor(private adapter: VaultFileAdapter) {}

  async load(): Promise<StoredAidianSettings> {
    ...
    if (
      settingsPath !== AIDIAN_SETTINGS_PATH
      || (
  ...
  async save(settings: StoredAidianSettings): Promise<void> {
    const content = JSON.stringify(
      stripLegacyFields(settings),
      null,
      2,
    );
    await this.adapter.write(AIDIAN_SETTINGS_PATH, content);
    await this.deleteLegacyFileIfPresent();
  }

  async exists(): Promise<boolean> {
    if (await this.adapter.exists(AIDIAN_SETTINGS_PATH)) {
      return true;
    }

    return this.adapter.exists(LEGACY_AIDIAN_SETTINGS_PATH);
  }

  async update(updates: Partial<StoredAidianSettings>): Promise<void> {
  ...
  private getDefaults(): StoredAidianSettings {
    return DEFAULT_AIDIAN_SETTINGS;
  }

  private async getLoadPath(): Promise<string | null> {
    if (await this.adapter.exists(AIDIAN_SETTINGS_PATH)) {
      return AIDIAN_SETTINGS_PATH;
    }

    if (await this.adapter.exists(LEGACY_AIDIAN_SETTINGS_PATH)) {
      return LEGACY_AIDIAN_SETTINGS_PATH;
    }
  ...
  private async deleteLegacyFileIfPresent(): Promise<void> {
    if (await this.adapter.exists(LEGACY_AIDIAN_SETTINGS_PATH)) {
      await this.adapter.delete(LEGACY_AIDIAN_SETTINGS_PATH);
    }
  }
}
