...
import {
  ...
  AidianSettingsStorage,
  LEGACY_AIDIAN_SETTINGS_PATH,
} from '@/providers/claude/storage/AidianSettingsStorage';
...
describe('AidianSettingsStorage', () => {
  let storage: AidianSettingsStorage;
  ...
  beforeEach(() => {
    ...
    storage = new AidianSettingsStorage(mockAdapter);
  });
  ...
  describe('load', () => {
    ...
    it('loads legacy .claude settings and migrates them to .aidian', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === LEGACY_AIDIAN_SETTINGS_PATH
      ));
      mockAdapter.read.mockImplementation(async (path: string) => {
        if (path === LEGACY_AIDIAN_SETTINGS_PATH) {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith(LEGACY_AIDIAN_SETTINGS_PATH);
    });
  ...
  describe('exists', () => {
    it('should return true when the new file exists', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === AIDIAN_SETTINGS_PATH
      ));

      const result = await storage.exists();

      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(AIDIAN_SETTINGS_PATH);
    });
    ...
    it('should return true when only the legacy file exists', async () => {
      ...
      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(AIDIAN_SETTINGS_PATH);
      expect(mockAdapter.exists).toHaveBeenCalledWith(LEGACY_AIDIAN_SETTINGS_PATH);
    });
